...
class Shape(object):
    ...
    def getShapeRotationVertex(self, orig=False):
        """
        :orig:    whether to use the original coordiantes (for mathemtatical computations)
                  or to use the rotated coordinates  (for getting the current position on canvas)
        ...
        pts = self.pointsWithoutRotation if orig else self.points
    ...
    def drawRotateVertex(self, path):
        vertex_info_orig= self.getShapeRotationVertex(True)
        vertex_info= self.getShapeRotationVertex()
        if vertex_info is not None:
            assert(vertex_info_orig is not None)
    ...
    def getCenter(self, orig=True):
        """
        :param orig:    whether to use the original parameters or the rotated ones.
        ...
        """
        p = self.points if orig else self.pointsWithoutRotation
    ...
    def getClosestVertex(self, point, epsilon):
        ...
        if self.selected:
            rot_info = self.getShapeRotationVertex()
    ...
    def applyRotationAngle(self, angle, shape_center, fromUnrotated=True):
        ...
        """
        # here i made a discretization for not having too much freedom of rotation
        # this could be free to be configured
        if 0:
            amountOfSteps = 50
